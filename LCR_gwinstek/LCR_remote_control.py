...
class LCR_8110G:
    ...
    def set_freq(self, freq):
        ...
        return self.sent_cmd(cmd, read_response=False)
...
def soilSample(label, port):
    ...
    log_path = get_log_path(label, prefix = "soil_")

    with LCR_8110G(port) as lcr:
        with open(log_path, "w", newline="") as csv_file:
            csv_writer = csv.writer(csv_file, delimiter=",")
            csv_writer.writerow(["Frequency [Hz]", "Capacitance [F]", "Dissipation factor [-]", "Impedance [ohm]", "Angle [deg]"])
            ...
            for f in freq_list:
                ...
                csv_writer.writerow([set_freq, C, D, Z, A])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--label', help='Specify a measurement label', default="test")
